...
def process_batch(batch):
    ...
    time.sleep(n)
    return [
        "task %s batch took: %d seconds" % (p.correlation_id, n) for m, p, b in batch
    ]
...
with build_blocking_connection_with_retry() as connection:
    with connection.channel() as channel:
        ...
        for batch in iterable_to_batches(
            channel.consume(queue="rpc_queue", inactivity_timeout=1),
            ...
            if len(batch) > 0:
                ...
                for (method_frame, properties, body), result in zip(batch, results):
                    print("delivered: %s" % properties.correlation_id)
                    sys.stdout.flush()
                ...
                print("acknowledged up to %s" % str(tag))
                sys.stdout.flush()
